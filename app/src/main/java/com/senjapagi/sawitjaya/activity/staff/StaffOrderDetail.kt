...
//        e.putExtra("status",orderData[position].status)
    ...
    fun successDialog(title: String, content: String, code: Int) {
        ...
        xSuccess.animation =
            AnimationUtils.loadAnimation(this@StaffOrderDetail, R.anim.item_animation_falldown)
        xSuccessTitle.text = title
        xSuccessContent.text = content
        xSuccessPButton.setOnClickListener {
            xSuccess.visibility = View.GONE
            xSuccess.animation =
                AnimationUtils.loadAnimation(this@StaffOrderDetail, R.anim.item_animation_fallup)
            when (code) {
                0 -> {
                    //Do Nothing
                }
    ...
    private fun getDetail() {
        try {
            ...
            AndroidNetworking.get(api.STAFF_ORDER_DETAIL + intent.getStringExtra("id"))
                .addHeaders("token", Preference(this).getPrefString(const.TOKEN))
                .build()
                .getAsJSONObject(object : JSONObjectRequestListener {
                    override fun onResponse(response: JSONObject) {
