...
import kotlinx.android.synthetic.main.fragment_order_staff_cancelled.*
 ...
 */
class orderStaffCancelled : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        getOrder()
        staffSrlOrderFailed.setOnRefreshListener {
            staffSrlOrderFailed.isRefreshing=false
            getOrder()
        }
    }
    ...
    private fun getOrder() {
        try {
            xEmptyState.visibility=View.GONE
            xInternetState.visibility=View.GONE
            recyclerViewStaffFailedOrder.setHasFixedSize(true)
            recyclerViewStaffFailedOrder.layoutManager = LinearLayoutManager(
            ...
            AndroidNetworking.get(api.STAFF_ORDER_FAILED)
                ...
                .getAsJSONObject(object : JSONObjectRequestListener {
                    override fun onResponse(response: JSONObject) {
                        ...
                        if(raz.length()<1){
                            staffFailedOrderErrorPlaceHolder?.visibility=View.VISIBLE
                            staffFailedOrderErrorPlaceHolder?.text="Belum Ada Request Jemput Batal/Gagal"
                            xEmptyState.visibility=View.VISIBLE
                            xEmptyMessage.text="Belum Ada Request Jemput Batal/Gagal"
                            xEmptyButton.setOnClickListener {
                                getOrder()
                            }
                        }
                        if (response.getBoolean("success_status")) {
                            ...
                            staffFailedOrderErrorPlaceHolder?.text = "Terjadi Error yang tidak diketahui"
                        }
                    }

                    override fun onError(anError: ANError?) {
                        anim_loading?.visibility=View.GONE
                        staffFailedOrderErrorPlaceHolder?.text = "Gagal Terhubung Dengan Server \n ${anError.toString()}"
                        xInternetState.visibility=View.VISIBLE
                        xRefreshInternetButton.setOnClickListener {
                            getOrder()
                        }
                    }
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_order_staff_cancelled, container, false)
    }
